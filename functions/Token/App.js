...
function parseDate(dateString) {
  ...
  const date = new Date(year, month - 1, day, hours, minutes, Math.floor(seconds), milliseconds);
  return date;
}
...
app.get('/getShopOrders/:shopId', async (req, res) => {
  try {
    ...
    const shopOrderRef = db.collection('shop').doc(shopId).collection('shopOrders');
